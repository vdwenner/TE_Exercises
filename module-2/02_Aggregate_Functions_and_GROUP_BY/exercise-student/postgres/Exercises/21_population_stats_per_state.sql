...
-- (4 rows)
SELECT DISTINCT AVG(population) AS average_population, MAX(population) AS max_population, MIN(population) AS min_population, census_region FROM state WHERE census_region IS NOT NULL GROUP BY census_region ORDER BY average_population ASC;